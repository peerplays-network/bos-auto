...
class CreateTrigger(Trigger):
    ...
    def getIncidentEvent(self):
        ...
        try:
            ...
            try:
                log.info(
                    "Creating event with teams {} in group {}.".format(
                        str(self.teams), self.eventgroup.identifier
                    )
                )
                ...
                msg = "The event with teams {} in group {} cannot open yet: {}".format(
                    str(self.teams), self.eventgroup.identifier, str(e)
                )
                log.info(msg)
    ...
    def createEvent(self):
        ...
        if not self.event.can_open:
            ...
            self.set_incident_status(
                status_name="postponed", status_expiration=can_open_by
            )
    ...
    def testConditions(self, *args, **kwargs):
        ...
        if self.start_time.replace(tzinfo=None) < datetime.utcnow():
            raise exceptions.CreateIncidentTooOldException(
                "Won't create event. " "'start_time' was {}".format(self.start_time)
            )
